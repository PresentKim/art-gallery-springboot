...
<t:layout>
    <jsp:attribute name="head">
        <title>${empty noticeEntity ? '소식지 등록' : '소식지 수정 :: '}${noticeEntity.nseq}</title>
...
<div id="notice_update_box">
    ...
    <div class="notice_update_innerbox">
        <form id="notice-form" class="notice-form" method="post" data-nseq="${noticeEntity.nseq}">
            <div class="notice_update_field">
                <label for="id">작성자</label> <input type="text" name="id" id="id" value="${loginMember.id}" readonly/>
            </div>
            <div class="notice_update_field">
                <label for="title">제목</label> <input type="text" name="title" id="title" value="${noticeEntity.title}"/>
            </div>
            <div class="notice_update_field">
                <label for="content">내용</label>
                <textarea name="content" id="content" rows="10" cols="100">${noticeEntity.content}</textarea>
            </div>
            <div class="notice_update_field">
                <label for="category">카테고리</label>
                <select name="category" id="category">
                    <c:if test="${empty noticeEntity}">
                    ...
                    <c:forEach items="${NoticeCategory.writableValues()}" var="c">
                        ...
                        <option value="${c.value}"
                                <c:if test="${c.isEquals(noticeEntity.category)}">selected</c:if>>${c.korName}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="notice_update_field_btn">
                <input type="submit" value="${empty noticeEntity ? '등록' : '수정'}">
